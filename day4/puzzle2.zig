...
pub fn main() !void {
    ...
    while (try r.readUntilDelimiterOrEof(&buf, '\n')) |line| {
        ...
        if ((try std.fmt.parseInt(u32, elf1_range1, 10) <= try std.fmt.parseInt(u32, elf2_range2, 10) and try std.fmt.parseInt(u32, elf2_range1, 10) <= try std.fmt.parseInt(u32, elf1_range2, 10)))
            sum += 1;
    }
    print("{d}\n", .{sum});
}
