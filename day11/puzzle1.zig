...
pub fn main() !void {
    ...
    //    var monkeys: [4]std.ArrayList(usize) = undefined;
    //    inline for (monkeys) |_, index| {
    //        monkeys[index] = std.ArrayList(usize).init(allocator);
    //        defer monkeys[index].deinit();
    //    }
    //    try monkeys[0].insertSlice(0, &[_]usize{ 98, 79 });
    //    try monkeys[1].insertSlice(0, &[_]usize{ 74, 75, 65, 54 });
    //    try monkeys[2].insertSlice(0, &[_]usize{ 97, 60, 79 });
    //    try monkeys[3].insertSlice(0, &[_]usize{74});
    //    var inspected: [4]usize = .{0} ** 4;
    //
    var round: usize = 0;
        ...
        for (monkeys) |*monkey, index| {
            while (monkey.popOrNull()) |a| {
                ...
                switch (index) {
                    ...
                    //                        } else try monkeys[3].insert(0, item);
                    //                    },
                    //                    1 => {
                    //                        item += 6;
                    //                        item /= 3;
